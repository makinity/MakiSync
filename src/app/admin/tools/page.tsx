...
type Tool = { id: number; name: string; logo_url: string | null; category: string | null; order: number };
...
export default function ToolsPage() {
  ...
  const load = () => {
    setLoading(true);
    fetch('/api/tools').then(r => r.json()).then(d => { setTools(d); setLoading(false); });
  };
  useEffect(load, []);
  ...
  const openEdit = (t: Tool) => { setEditing(t); setForm({ name: t.name, logo_url: t.logo_url ?? '', category: t.category ?? '' }); setShowForm(true); };

  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]; if (!file) return;
    setUploading(true);
    try { const url = await uploadFile('general', file, `tools/${Date.now()}-${file.name}`); setForm(f => ({ ...f, logo_url: url })); }
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    if (editing) {
      await fetch(`/api/tools/${editing.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) });
    } else {
  ...
  return (
    <AppLayout title="Tools" description="Admin">
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', alignItems: 'center' }}>
          <div style={{ position: 'relative', flex: 1, minWidth: 160 }}>
            ...
            <input type="text" placeholder="Search tools..." value={search} onChange={e => setSearch(e.target.value)} style={{ ...inp, paddingLeft: '2.25rem' }} />
          </div>
          ...
          <button onClick={openCreate} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', padding: '0.5rem 1rem', borderRadius: 10, border: 'none', background: 'var(--admin-accent)', color: '#fff', fontSize: '0.85rem', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' }}>
            <i className="bi bi-plus-lg" /> Add Tool
          </button>
        </div>
        ...
        {loading ? (
          ...
          <div style={{ padding: '3rem 2rem', textAlign: 'center', background: 'var(--admin-card)', border: '1px solid var(--admin-border)', borderRadius: 'var(--admin-radius)' }}>
            <i className="bi bi-tools" style={{ fontSize: '2.5rem', opacity: 0.2, color: 'var(--admin-accent)', display: 'block', marginBottom: '0.75rem' }} />
            <div style={{ fontSize: '0.9rem', fontWeight: 600, color: 'var(--admin-text-secondary)' }}>{search ? 'No tools match your search' : 'No tools yet'}</div>
            <div style={{ fontSize: '0.8rem', color: 'var(--admin-text-muted)', marginTop: '0.25rem' }}>Click "Add Tool" to get started.</div>
          </div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: '0.85rem' }}>
            {filtered.map(t => {
              ...
              return (
                <div key={t.id} style={{ background: 'var(--admin-card)', border: '1px solid var(--admin-border)', borderRadius: 14, padding: '1.25rem 1rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.65rem', boxShadow: 'var(--admin-shadow)', transition: 'border-color 0.15s', textAlign: 'center' }}
                  onMouseEnter={e => (e.currentTarget.style.borderColor = 'var(--admin-border-strong)')}
                  onMouseLeave={e => (e.currentTarget.style.borderColor = 'var(--admin-border)')}>

                  <div style={{ width: 56, height: 56, borderRadius: 14, overflow: 'hidden', background: cc.bg, border: `1px solid ${cc.color}33`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {t.logo_url
                      ? <img src={t.logo_url} alt={t.name} style={{ width: '80%', height: '80%', objectFit: 'contain' }} />
                      : <i className="bi bi-tools" style={{ fontSize: '1.4rem', color: cc.color }} />}
                  </div>

                  <div style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--admin-text-primary)', lineHeight: 1.2 }}>{t.name}</div>
                  ...
                  <div style={{ display: 'flex', gap: '0.4rem', marginTop: 'auto' }}>
                    <button onClick={() => openEdit(t)} style={{ padding: '0.3rem 0.65rem', borderRadius: 8, border: '1px solid var(--admin-border-strong)', background: 'transparent', color: 'var(--admin-text-muted)', fontSize: '0.72rem', cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                      <i className="bi bi-pencil-fill" /> Edit
                    </button>
                    <button onClick={() => setDeleteTarget(t)} style={{ padding: '0.3rem 0.65rem', borderRadius: 8, border: '1px solid rgba(239,68,68,0.3)', background: 'rgba(239,68,68,0.08)', color: '#f87171', fontSize: '0.72rem', cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
      ...
      {showForm && (
        <FormModal title={editing ? 'Edit Tool' : 'New Tool'} onClose={() => setShowForm(false)} onSubmit={handleSave} loading={saving} submitLabel={editing ? 'Update' : 'Add'}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div>
              <label style={lbl}>Logo / Icon</label>
              <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                <div style={{ width: 64, height: 64, borderRadius: 14, overflow: 'hidden', background: 'rgba(59,130,246,0.08)', border: '1px solid var(--admin-border)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }} onClick={() => fileRef.current?.click()}>
                  {form.logo_url
            ...
            <div>
              <label style={lbl}>Tool Name <span style={{ color: '#f87171' }}>*</span></label>
              <input type="text" placeholder="e.g. Notion, Slack, Trello…" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} style={inp} autoFocus />
            </div>

            <div>
              <label style={lbl}>Category</label>
              <input type="text" placeholder="e.g. Project Management, Communication…" value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))} style={inp} list="tool-cat-suggestions" />
              <datalist id="tool-cat-suggestions">
                {Array.from(new Set(tools.map(t => t.category).filter(Boolean))).map(c => (

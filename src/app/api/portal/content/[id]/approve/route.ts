...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Verify client owns this content
  ...
  try {
    ...
    await client.query(`UPDATE content_items SET status = 'approved', updated_at = NOW() WHERE id = $1`, [id]);
    if (body.comment) {
      await client.query(
        `INSERT INTO content_comments (content_item_id, author_id, body) VALUES ($1, $2, $3)`,
    ...
    await client.query(
      `INSERT INTO notifications (recipient_id, type, title, body, reference_id, reference_type)
       SELECT ci.created_by, 'content_approved', 'Content Approved', $2, $1, 'content_item'
       FROM content_items ci WHERE ci.id = $1`,
      [id, `Content "${contentRows[0].title}" was approved`]
    );
